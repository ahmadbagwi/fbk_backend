          ...
          <template #laman-admin>
            <div class="md:flex w-full md:inline-block block mx-1 md:mx-1 p-2 rounded bg-green-800 text-white font-semibold">
              <div class="w-5/5 md:w-full mb-1 sm:mb-0 flex justify-between">
                <div class="mx-4 w-auto max-w-full berita-create inline flex p-1">
                  <span class="mr-4">Slider Halaman Utama</span>
                  <inertia-link href="/admin/slider/create/0">
            ...
            <vue-good-table
              ...
              <template slot="table-row" slot-scope="props">
                <div v-if="props.column.field == 'slide'" class="flex flex-wrap">
                  <div v-for="list in props.row.nilai.split(',')" :key="list">
                    <img :src="list" class="w-20 m-2">
                  </div>
                </div>
                <span v-if="props.column.field == 'edit'">
                  <div class="input-group-btn">
                      <button class="flex text-green-400 border-green-400  px-1 py-1 rounded" >
                        &#9997;
                        <inertia-link :href="`/admin/slider/create/${props.row.id}`">
                          Edit
                        </inertia-link>
                      </button>
                  </div>
...
import Input from '../../Jetstream/Input.vue'
  ...
  export default {
    ...
    created () {
      if (this.data.nilai) {
        this.array_slide.push(this.data.nilai.split(","))
      }
    },
    methods: {
      destroy(id){
        ...
        if(hapus) {
          axios.post('/admin/slider/destroy', {
            id : this.id
          })
            .then((response) => {
